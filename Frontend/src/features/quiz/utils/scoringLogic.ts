...
export const evaluateResult = (rawScores: QuizScores): ResultCategory => {
  ...
  const scores = [
    { domain: 'ADHD', value: percentages.ADHD },
    { domain: 'AUTISM', value: percentages.AUTISM },
    { domain: 'ID', value: percentages.ID },
  ];
  ...
  // If top two are tied and third is lower
  if (
    highest.value === second.value &&
  ...
  if (highest.domain === 'ADHD') return 'ADHD Traits';
  if (highest.domain === 'AUTISM') return 'Autism Traits';
  return 'Intellectual Disability Traits';
};
