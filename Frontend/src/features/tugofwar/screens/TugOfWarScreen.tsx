...
// ─── Stick figures (simplified SVG-like shapes using Views) ──────
...
const TugOfWarScreen = ({ route, navigation }: any) => {
    ...
    const handleAISubmit = useCallback(async () => {
        ...
        // Parse the correct answer from the question to decide if AI gets it right (~60%)
        const isCorrect = Math.random() < 0.6;
        // Extract numbers from question like "7 + 4 = ?"
        let aiAnswer: number;
        try {
            const qPart = team2Q.replace('= ?', '').trim();
            const correctVal = eval(qPart.replace('×', '*'));
    ...
    const finishGame = useCallback(() => {
        ...
        setTeam1Score((s1) => {
            setTeam2Score((s2) => {
                const currentRope = ropeX.value;
                let w = 'draw';
                if (currentRope < 0) w = 'team1';
                else if (currentRope > 0) w = 'team2';

                // Save to backend
                if (user?.token && gameIdRef.current) {
                    endGame(user.token, gameIdRef.current, s1, s2, w, GAME_DURATION).catch(() => { });
                }
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [user?.token, difficulty, navigation]);
